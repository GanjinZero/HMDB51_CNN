...
def weight_variable(shape):
    initial = tf.truncated_normal(shape, mean = 0, stddev = 0.04)
    return tf.Variable(initial)

def bias_variable(shape):
    #initial = tf.constant(0.1, shape = shape)
    initial = tf.truncated_normal([1], mean = 0, stddev = 0.04)
    return tf.Variable(initial)
...
W_fc1 = weight_variable([5*12*64*5, 2048])
...
h_pool2_flat = tf.concat([h_pool21_flat,h_pool22_flat,h_pool23_flat,h_pool24_flat,h_pool25_flat],axis=1)
...
for i in range(max_steps):
    ...
    if (i % 10) == 0:
        train_accuracy,crs = sess.run([accuracy,cross_entropy],feed_dict = {x_holder:x_b, y_holder:y_b, keep_prob:1.0})
        print("step %d, train_accuracy %g" %(i, train_accuracy))
        print("cross entropy on train %g" %crs)
        ...
        #print(top_k_op)
        acc,acck=sess.run([accuracy,acc_k],feed_dict = {x_holder:imageset_test, y_holder:classset_test,keep_prob:1.0})
